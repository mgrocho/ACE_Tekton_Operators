...
CRs_generated_folder=/Users/michal.grochowski/cp4idemo/cp4i_CRs_generated
...
#UWAGA! oryginalnie zastosowane bylo "base64 -w 0", zmienilem na "base64"
...
then
    ...
    barauth=$(base64 -i ${DIRbarauth}/auth.json)
...
# Create CR for odbc.ini , if folder exists and is ot empty
if [ -d "${DIRodbcini}" ]
then
	if [ "$(ls -A ${DIRodbcini})" ]; then
    echo "Generating odbcini CR yaml"
    odbcini=$(base64 -i ${DIRodbcini}/odbc.ini)
...
then
    ...
    generic=$(base64 -i ${PathToConfigFolder}/generic.zip)
...
then
    ...
    setdbparms=$(base64 -i ${DIRsetdbparms}/setdbparms.txt)
...
then
    ...
    truststore=$(base64 -i server-config/initial-config/truststore/cert.p12)
...
then
    ...
    policy=$(base64 -i ${PathToConfigFolder}/policy.zip)
...
then
    ...
    serverconf=$(base64 -i ${DIRserverconf}/server.conf.yaml)
